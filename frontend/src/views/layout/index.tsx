...
import { useSettingStore, useRootVarsStore } from '../stores'
...
function App(props: any) {
  ...
  const { getRootVars, isThemeToggle } = useRootVarsStore()
  const rootVras = getRootVars()
    ...
    <div className='w-full h-full flex flex-col'>
      <div className='title-bar flex-shrink-0 w-full h-[42px] bg-[#202124] text-[#f1f1f2] flex justify-between items-center pl-[20px] box-border'>
        ...
        <div className='operator h-full flex items-center cursor-default'>
          <Button type='text' onClick={isThemeToggle}>
            {rootVras.isTheme === 'dark' ? '暗色' : '亮色'} 还没搞完
          </Button>

          <div className='h-full hover:bg-[#3a3a3d] leading-[42px] px-[14px]' onClick={handleOpenChrome}>
            <GlobalOutlined />
          </div>
          <div className='h-full hover:bg-[#3a3a3d] leading-[42px] px-[14px]' onClick={handleMinimizeWindow}>
            <MinusOutlined />
          </div>
          <div className='h-full hover:bg-[#3a3a3d] leading-[42px] px-[14px]' onClick={handleMaxOrUnMaxWindow}>
            {fullScreen ? <FullscreenExitOutlined /> : <FullscreenOutlined />}
          </div>
          <div className='h-full hover:bg-[#3a3a3d] leading-[42px] px-[14px]' onClick={handleCloseApp}>
            <CloseOutlined />
          </div>
        </div>
      </div>
      <div className='flex flex-1' style={{ height: 'calc(100% - 42px)' }}>
        ...
        <div className='flex-1 bg-[#181818] overflow-y-auto f-beautify-scrollbar'>{props.children}</div>
      </div>
    </div>
  )
}
